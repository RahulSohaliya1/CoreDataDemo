...
//                navigationController?.pushViewController(employeeListViewController, animated: true)
    ...
    private func deleteUsers() {
        ...
        let viewContext = appDelegate.persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>.init(entityName: "Students")
        //fetchRequest.predicate = NSPredicate(format: "salary = %f", Float(salaryTextFiled.text ?? "0.0") ?? 0.0)
//        fetchRequest.predicate = NSPredicate(format: "name = %@", nameTextFiled.text ?? "0.0")
...
//                tempStudent.setValue("3456789", forKey: "address")
                
                //tempStudent.setValue(Double(salaryTextFiled.text ?? "0"), forKey: "salary")
                try viewContext.save()
                print("Data Saved Successfully")
